use crate::{
    ...
    policy::Policy,
    token::{Command, Did, DidType, JsonObject, NucToken, TokenBody},
};
...
impl NucTokenBuilder {
    ...
    pub fn build(self, issuer_key: &SigningKey) -> Result<String, NucTokenBuildError> {
        ...
        let issuer = Did::new(public_key, DidType::Nil);
        let mut token =
...
mod tests {
    ...
    fn extend_token() {
        ...
        let next = NucTokenBuilder::extending(base.clone())
            ...
            .audience(Did::new([0xdd; 33], DidType::Nil))
            .build(&key.into())
            .expect("build failed");
    ...
    fn encode_decode() {
        ...
        let issuer = Did::new(issuer, DidType::Nil);
